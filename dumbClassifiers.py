    ...
    def train(self, X, Y):
        ...
        for y in Y:
            if y > 0:
                ...
                negativeCount += 1
        self.mostFrequentClass = 1 if positiveCount > negativeCount else -1
        return
...
class FirstFeatureClassifier(BinaryClassifier):
    ...
    def train(self, X, Y):
        ...
        for index, x in enumerate(X):
            if x[0] > 0:
                positiveVote += Y[index]
            else:
                negativeVote += Y[index]
        self.classForPos = 1 if positiveVote > 0 else -1
        self.classForNeg = 1 if negativeVote > 0 else -1
